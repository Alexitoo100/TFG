...
$consulta="SELECT juegos.id, juegos.nombre, precio, descripcion, genero, n_jugadores, img, certificaciones, p.nombre, d.nombre, f_lanzamiento FROM juegos JOIN juegos_plataformas 
AS jp on juegos.id=jp.id_juegos JOIN plataformas AS p on p.id=jp.id_plataformas 
JOIN juegos_desarrolladoras AS jd on juegos.id=jd.id_juegos JOIN desarrolladoras AS d on d.id=jd.id_desarrolladoras";
...
foreach ($consulta_fin as $juegos) {
    ...
    "img" => $juegos[6], "certificaciones" => $juegos[7], "plataforma" => $juegos[8], "desarrolladora" => $juegos[9], "f_lanzamiento" => $juegos[10]);
